...
std::shared_ptr<LZ4Sequence> SmallLZ4::setSequence(uint64_t litLength, uint64_t matchLength, uint64_t matchOffset, const unsigned char* data, bool lastToken)
{
...
std::shared_ptr<LZ4Sequence> SmallLZ4::getSequence(const unsigned char* in_data, uint64_t blockSize)
{
   sequenceCall++;
   ...
   return std::make_shared<SmallLZ4>(numLiterals, matchLength - MinMatchLength, matchOffset, literalData, false);
}
#pragma optimize( "", on )